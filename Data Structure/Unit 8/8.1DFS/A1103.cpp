...
//init函数预处理fac数组，注意把0也算进去
void init()
{
    int i=0,temp=0;
    while(temp<=n){
    //当i^p没有超过n时候，不断把i^p加入fac
        fac.push_back(temp);
        temp=power(++i);
    }
}
//DFS函数，当前访问的fac[index],nowk为当前选中个数
//sum为当前选中的个数之和，facSum为当前选中的底数之和
void DFS(int index,int nowK,int sum,int facSum){
    ...
    if(index-1>=0){
        //fac[0]不需要选择
        temp.push_back(index);      //把底数index加入刀临时序列temp
        DFS(index,nowK+1,sum+fac[index],facSum+index);      //“选”的分支
        temp.pop_back();        //"选"的分支结束后，把刚刚加进去的数pop掉
